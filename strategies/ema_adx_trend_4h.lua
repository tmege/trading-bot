--[[
  EMA + ADX Trend Following Strategy — Generic Multi-Coin

  Trend following on 4h timeframe using EMA crossover + ADX strength filter.
  The COIN global is injected by the C engine before loading.
  Each instance runs independently with its own state.

  Logic:
  - LONG: EMA(12) > EMA(26) + ADX > 25 (strong trend) + price > SMA(200)
  - SHORT: EMA(12) < EMA(26) + ADX > 25 + price < SMA(200)
  - Exit: EMA cross inverse OR ADX < 20 (trend fading)
  - Wider stops for 4h TF: SL 2.5%, TP 6%
  - Lower leverage: 3x (trend trades need room)
  - Size: $40
]]
...
local config = {
    ...
    -- Entry conditions
    adx_entry     = 25,          -- ADX must be above this to enter (strong trend)
    adx_exit      = 20,          -- exit if ADX drops below this (trend fading)
    ...
    -- Exit targets
    tp_pct        = 6.0,         -- wider TP for trend following
    sl_pct        = 2.5,         -- wider SL for 4h swings

    -- Timing
    check_sec     = 30,
    cooldown_sec  = 600,         -- 10 min cooldown (4h TF = fewer signals)
    max_hold_sec  = 86400,       -- max hold 24h

    enabled       = true,
}

-- Instance name (e.g. "ema_adx_trend_4h_eth")
local instance_name = "ema_adx_trend_4h_" .. config.coin:lower()
...
function on_init()
    bot.log("info", string.format(
        "%s: EMA+ADX Trend 4h, ADX entry>%d exit<%d, SL=%.1f%%, TP=%.1f%%, lever=%dx",
        instance_name, config.adx_entry, config.adx_exit,
        config.sl_pct, config.tp_pct, config.leverage))
...
function on_tick(coin, mid_price)
    ...
    local ema_12 = ind.ema_12
    local ema_26 = ind.ema_26
    local adx = ind.adx
    local sma_200 = ind.sma_200

    if not ema_12 or not ema_26 or not adx then
        bot.log("warn", string.format("%s: missing indicator fields (ema12=%s ema26=%s adx=%s)",
            instance_name, tostring(ema_12), tostring(ema_26), tostring(adx)))
        return
    end
    ...
    if in_position then
        ...
        -- Exit condition 1: EMA cross inverse
        local ema_bearish_cross = ema_12 < ema_26
        local ema_bullish_cross = ema_12 > ema_26

        if position_side == "long" and ema_bearish_cross then
            close_position(string.format("EMA bearish cross (EMA12=%.2f < EMA26=%.2f)", ema_12, ema_26))
            last_trade = now
            return
        end
        if position_side == "short" and ema_bullish_cross then
            close_position(string.format("EMA bullish cross (EMA12=%.2f > EMA26=%.2f)", ema_12, ema_26))
            last_trade = now
            return
        end

        -- Exit condition 2: ADX fading below threshold
        if adx < config.adx_exit then
            close_position(string.format("ADX fading (%.1f < %d)", adx, config.adx_exit))
            last_trade = now
            return
        end
    ...
    -- SMA 200 may not be available (needs 200+ bars of history)
    -- If unavailable, skip the SMA filter but log it
    local has_sma200 = sma_200 and sma_200 > 0

    bot.log("debug", string.format(
        "%s: %s=$%.2f EMA12=%.2f EMA26=%.2f ADX=%.1f SMA200=%s",
        instance_name, config.coin, mid_price, ema_12, ema_26, adx,
        has_sma200 and string.format("%.2f", sma_200) or "N/A"))

    -- Need strong trend
    if adx < config.adx_entry then
        bot.log("debug", string.format("%s: ADX too low (%.1f < %d), no trend",
            instance_name, adx, config.adx_entry))
        return
    end

    -- LONG: EMA(12) > EMA(26) + ADX > 25 + price > SMA(200)
    if ema_12 > ema_26 then
        if not has_sma200 or mid_price > sma_200 then
            bot.log("info", string.format(
                "%s: TREND LONG — EMA12=%.2f > EMA26=%.2f, ADX=%.1f, price=$%.2f %s → LONG",
                instance_name, ema_12, ema_26, adx, mid_price,
                has_sma200 and string.format("> SMA200=%.2f", sma_200) or "(SMA200 N/A)"))
            ...
            bot.log("debug", string.format("%s: EMA bullish but price ($%.2f) below SMA200 ($%.2f)",
                instance_name, mid_price, sma_200))
        end
    end
    ...
    if ema_12 < ema_26 then
        if not has_sma200 or mid_price < sma_200 then
            bot.log("info", string.format(
                "%s: TREND SHORT — EMA12=%.2f < EMA26=%.2f, ADX=%.1f, price=$%.2f %s → SHORT",
                instance_name, ema_12, ema_26, adx, mid_price,
                has_sma200 and string.format("< SMA200=%.2f", sma_200) or "(SMA200 N/A)"))
            ...
            bot.log("debug", string.format("%s: EMA bearish but price ($%.2f) above SMA200 ($%.2f)",
                instance_name, mid_price, sma_200))
        end
    end
end
