--[[
  Williams %R + OBV Swing Strategy — Generic Multi-Coin

  Swing strategy combining Williams %R extremes with On-Balance Volume confirmation.
  The COIN global is injected by the C engine before loading.
  Each instance runs independently with its own state.

  Logic:
  - LONG: Williams %R < -80 (oversold) + OBV rising (obv > obv_sma) + price > SMA(50)
  - SHORT: Williams %R > -20 (overbought) + OBV falling (obv < obv_sma) + price < SMA(50)
  - Exit LONG: %R reaches -50 or OBV diverges (obv < obv_sma)
  - Exit SHORT: %R reaches -50 or OBV > obv_sma
  - SL: 2%, TP: 5%, Lever: 3x, Size: $40
]]
...
local config = {
    ...
    -- Williams %R thresholds (loosened from -80/-20)
    wr_oversold   = -70,        -- long entry below this
    wr_overbought = -30,        -- short entry above this
    wr_exit       = -50,        -- exit when %R crosses toward neutral
    ...
    -- Timing
    check_sec     = 30,         -- check every 30s (swing pace)
    cooldown_sec  = 600,        -- 10 min between trades
    max_hold_sec  = 28800,      -- max hold 8h

    enabled       = true,
}

-- Instance name (e.g. "williams_obv_4h_eth")
local instance_name = "williams_obv_4h_" .. config.coin:lower()
...
function on_init()
    bot.log("info", string.format(
        "%s: WR[%d/%d], OBV+SMA50, SL=%.1f%%, TP=%.1f%%, check=%ds",
        instance_name, config.wr_oversold, config.wr_overbought,
        config.sl_pct, config.tp_pct, config.check_sec))
...
function on_tick(coin, mid_price)
    ...
    -- Get indicators (4h timeframe, 30 candles)
    local ind = bot.get_indicators(config.coin, "4h", 30, mid_price)
    if not ind then
        bot.log("warn", instance_name .. ": get_indicators returned nil")
        return
    end

    local wr       = ind.williams_r
    local obv      = ind.obv
    local obv_sma  = ind.obv_sma
    local sma20    = ind.sma_20  -- use SMA20 as trend filter (SMA50 not available)

    if not wr or not obv or not obv_sma then
        bot.log("warn", string.format("%s: missing fields — wr=%s obv=%s obv_sma=%s",
            instance_name, tostring(wr), tostring(obv), tostring(obv_sma)))
        return
    end

    bot.log("debug", string.format("%s: %s=$%.2f WR=%.1f OBV=%s OBV_SMA=%s SMA20=%.2f",
        instance_name, config.coin, mid_price, wr,
        tostring(obv), tostring(obv_sma), sma20 or 0))

    -- ── In-position: check conditional exits ──
    ...
    if in_position then
        ...
        if position_side == "long" then
            -- Exit LONG: %R reaches -50 or OBV diverges (obv < obv_sma)
            if wr >= config.wr_exit then
                close_position(string.format("WR reached %.0f (exit zone)", wr))
                last_trade = now
                return
            end
            if obv < obv_sma then
                close_position("OBV diverged below OBV SMA (volume fading)")
                last_trade = now
                return
            end
        elseif position_side == "short" then
            ...
            if obv > obv_sma then
                close_position("OBV rose above OBV SMA (buying pressure)")
                last_trade = now
                return
            end
        end
    ...
    -- LONG: WR oversold + OBV rising (SMA20 trend filter optional)
    if wr < config.wr_oversold and obv > obv_sma then
        -- SMA20 trend filter: skip if clearly below (price < 97% of SMA20)
        if sma20 and mid_price < sma20 * 0.97 then
            -- strong downtrend, skip long
        else
            bot.log("info", string.format(
                "%s: WR=%.0f (oversold) + OBV rising → LONG",
                instance_name, wr))
    ...
    -- SHORT: WR overbought + OBV falling
    if wr > config.wr_overbought and obv < obv_sma then
        if sma20 and mid_price > sma20 * 1.03 then
            -- strong uptrend, skip short
        else
            bot.log("info", string.format(
                "%s: WR=%.0f (overbought) + OBV falling → SHORT",
                instance_name, wr))
