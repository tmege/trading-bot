...
-- Instance name for logging and advisory (e.g. "scalp_eth", "scalp_btc")
local instance_name = "scalp_" .. config.coin:lower()
...
local function place_entry(side, mid)
    ...
    if oid then
        bot.log("info", string.format("%s: ENTRY %s @ $%.2f ($%.0f)",
            instance_name, string.upper(side), price, config.entry_size))
        return oid
    end
    return nil
end
...
function on_advisory(json_str)
    ...
    -- Only parse our own section (e.g. "scalp_eth": { ... })
