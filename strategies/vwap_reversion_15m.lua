--[[
  VWAP Mean Reversion Strategy — Generic Multi-Coin

  Mean reversion strategy on 15m timeframe.
  The COIN global is injected by the C engine before loading.
  Each instance runs independently with its own state.

  Logic:
  - LONG: price < VWAP - 2*ATR AND RSI < 35 (oversold below VWAP)
  - SHORT: price > VWAP + 2*ATR AND RSI > 65 (overbought above VWAP)
  - Target: return to VWAP (dynamic exit)
  - SL: 1%, max TP: 2%
  - Monitor VWAP for dynamic exit
]]
...
local config = {
    ...
    -- Entry conditions
    atr_mult      = 2.0,          -- deviation from VWAP = 2x ATR
    rsi_oversold  = 35,           -- RSI threshold for long
    rsi_overbought = 65,          -- RSI threshold for short
    ...
    -- Exit
    sl_pct        = 1.0,          -- tight stop loss 1%
    max_tp_pct    = 2.0,          -- max take profit 2% (cap)

    -- Timing
    check_sec     = 5,            -- check every 5s (fast)
    cooldown_sec  = 60,           -- 1 min between trades
...
local instance_name = "vwap_reversion_15m_" .. config.coin:lower()
...
local function place_sl_tp(fill_price, side, pos_size)
    ...
    if side == "long" then
        ...
        tp_price = fill_price * (1 + config.max_tp_pct / 100)
    ...
    bot.log("info", string.format("%s: SL=$%.2f (%.1f%%), TP_cap=$%.2f (+%.1f%%)",
        instance_name, sl_price, config.sl_pct, tp_price, config.max_tp_pct))
end
...
function on_init()
    bot.log("info", string.format(
        "%s: VWAP Reversion 15m, ATR_mult=%.1f, SL=%.1f%%, TP_cap=%.1f%%, lever=%dx, check=%ds",
        instance_name, config.atr_mult, config.sl_pct, config.max_tp_pct,
        config.leverage, config.check_sec))
...
function on_tick(coin, mid_price)
    ...
    -- Position management: dynamic VWAP exit
    if in_position then
        local pos = bot.get_position(config.coin)
        if not pos or pos.size == 0 then
            in_position = false
        ...
        -- Check if price has returned to VWAP (target)
        local ind = bot.get_indicators(config.coin, "15m", 50, mid_price)
        if ind and ind.vwap then
            local vwap = ind.vwap
            if position_side == "long" and mid_price >= vwap then
                close_position(string.format("VWAP target reached ($%.2f >= VWAP $%.2f)",
                    mid_price, vwap))
                last_trade = now
                return
            elseif position_side == "short" and mid_price <= vwap then
                close_position(string.format("VWAP target reached ($%.2f <= VWAP $%.2f)",
                    mid_price, vwap))
                last_trade = now
                return
    ...
    if not vwap or not atr or not rsi then
        bot.log("warn", string.format("%s: missing fields — vwap=%s atr=%s rsi=%s",
            instance_name, tostring(vwap), tostring(atr), tostring(rsi)))
        return
    end

    local lower_band = vwap - config.atr_mult * atr
    local upper_band = vwap + config.atr_mult * atr

    bot.log("debug", string.format(
        "%s: %s=$%.2f VWAP=$%.2f ATR=$%.2f bands=[%.2f / %.2f] RSI=%.1f",
        instance_name, config.coin, mid_price, vwap, atr, lower_band, upper_band, rsi))

    -- LONG: price below VWAP - 2*ATR + RSI oversold
    if mid_price < lower_band and rsi < config.rsi_oversold then
        bot.log("info", string.format(
            "%s: OVERSOLD below VWAP — price $%.2f < band $%.2f, RSI=%.1f → LONG",
            instance_name, mid_price, lower_band, rsi))
    ...
    -- SHORT: price above VWAP + 2*ATR + RSI overbought
    if mid_price > upper_band and rsi > config.rsi_overbought then
        bot.log("info", string.format(
            "%s: OVERBOUGHT above VWAP — price $%.2f > band $%.2f, RSI=%.1f → SHORT",
            instance_name, mid_price, upper_band, rsi))
...
function on_fill(fill)
    ...
    elseif fill.oid == tp_oid then
        bot.log("info", string.format("%s: TP cap hit — pnl=%.4f (win rate: %d/%d = %.0f%%)",
            instance_name, fill.closed_pnl, win_count, trade_count,
            trade_count > 0 and (win_count / trade_count * 100) or 0))
        if sl_oid then bot.cancel(config.coin, sl_oid); sl_oid = nil end
        tp_oid = nil
