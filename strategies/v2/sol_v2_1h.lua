--[[
  SOL v2 — Breakout/PSAR multi-signal scanner (1h)

  Signals from 644k-eval scan, walk-forward validated (70/30):
    L1: RSI>60 + bearish_engulf         → LONG  TP 4.0%/SL 3.0% (OOS: 65.6% WR, +8.2%/m)
    S1: dc_breakout_low + PSAR_bull     → SHORT TP 2.0%/SL 3.0% (OOS: 81.2% WR, +7.8%/m, DD 21%)
    S2: CCI<-100 + squeeze_on          → SHORT TP 3.0%/SL 2.0% (OOS: 69.6% WR, +6.6%/m, 23 trades)

  Config: x5 leverage, 15% equity = 75% exposure per trade.
...
-- Signal Definitions
local SIGNALS = {
    { name = "L1_rsi60_bearengulf", side = "long", tp = 4.0, sl = 3.0,
      check = function(ind, mid, h)
          if ind.rsi <= 60 then return false end
          local c = h.candles
          if not c or #c < 2 then return false end
          local prev, curr = c[#c - 1], c[#c]
          local prev_green = prev.close > prev.open
          local curr_red = curr.close < curr.open
          local engulf = curr.open > prev.close and curr.close < prev.open
          return prev_green and curr_red and engulf
      end },

    { name = "S1_dclow_psarbull", side = "short", tp = 2.0, sl = 3.0,
      check = function(ind, mid, h)
          return mid < ind.dc_lower and ind.psar_up
      end },

    { name = "S2_cciosq_squeeze", side = "short", tp = 3.0, sl = 2.0,
      check = function(ind, mid, h)
          return ind.cci_20 < -100 and ind.squeeze_on
      end },
}

local instance_name = "sol_v2_1h"
...
function on_init()
    ...
    if had == "true" then
        ...
        if pos and pos.size ~= 0 then
            ...
            entry_time = bot.time()
            bot.cancel_all_exchange(config.coin)
            if active_tp == 0 then active_tp = 2.0 end
...
function on_tick(coin, mid_price)
    ...
    if not ind.rsi or not ind.atr or not ind.macd_histogram or not ind.sma_20 or not ind.sma_50 then return end
    ...
    for _, sig in ipairs(SIGNALS) do
        ...
        if ok and matched then
            bot.log("info", string.format(
                "%s: SIGNAL %s (%s) RSI=%.0f MACD=%.4f SMA20=%.2f",
                instance_name, sig.name, sig.side,
                ind.rsi, ind.macd_histogram, ind.sma_20))
            active_tp = sig.tp
            active_sl = sig.sl
