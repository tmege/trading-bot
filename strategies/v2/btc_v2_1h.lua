--[[
  BTC v2 — Multi-signal MFI/LowVol scanner (1h)

  Signals from 644k-eval scan, walk-forward validated (70/30):
    L1: low_vol + MFI>80        → LONG  TP 3.0%/SL 1.5% (OOS: 68.8% WR, +11.1%/m)
    S1: RSI<35 + low_vol        → SHORT TP 3.0%/SL 1.5% (OOS: 64.5% WR, +15.4%/m)
    S2: shooting_star + MFI<30  → SHORT TP 3.0%/SL 2.5% (OOS: 80.0% WR, +14.8%/m)

  Config: x7 leverage, 30% equity = 210% exposure per trade.
...
-- Signal Definitions
local SIGNALS = {
    { name = "L1_lowvol_mfi80", side = "long", tp = 3.0, sl = 1.5,
      check = function(ind, mid, h)
          return (ind.atr / mid) < 0.005 and ind.mfi > 80
      end },

    { name = "S1_rsi35_lowvol", side = "short", tp = 3.0, sl = 1.5,
      check = function(ind, mid, h)
          return ind.rsi < 35 and (ind.atr / mid) < 0.005
      end },

    { name = "S2_shootstar_mfi30", side = "short", tp = 3.0, sl = 2.5,
      check = function(ind, mid, h)
          local c = h.candles
          ...
          local upper = last.high - math.max(last.open, last.close)
          local is_star = upper > 2 * body and body / range < 0.3
          return is_star and ind.mfi < 30
      end },
}

local instance_name = "btc_v2_1h"
...
function on_tick(coin, mid_price)
    ...
    if not ind.rsi or not ind.atr or not ind.mfi then return end
    ...
    for _, sig in ipairs(SIGNALS) do
        ...
        if ok and matched then
            bot.log("info", string.format(
                "%s: SIGNAL %s (%s) RSI=%.0f MFI=%.0f ATR%%=%.3f",
                instance_name, sig.name, sig.side,
                ind.rsi, ind.mfi, (ind.atr / mid_price) * 100))
            active_tp = sig.tp
            active_sl = sig.sl
