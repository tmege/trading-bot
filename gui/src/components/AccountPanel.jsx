...
export default function AccountPanel({ account }) {
  const data = account || { dailyPnl: 0, unrealizedPnl: 0, dailyFees: 0, dailyTrades: 0, balance: 0, cumulativePnl: 0 };
  ...
  const cumPnl = data.cumulativePnl || 0;
  const cumColor = cumPnl >= 0 ? 'text-profit' : 'text-loss';
  const CumIcon = cumPnl >= 0 ? TrendingUp : TrendingDown;
  ...
  return (
    <div className="bg-surface-card border border-surface-border rounded-lg p-4">
      ...
      <div className="grid grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div>
          <div className="flex items-center gap-1.5 mb-1">
            <CumIcon size={14} className={cumColor} />
            <span className="text-xs text-gray-500">Cumul P&L</span>
          </div>
          <span className={`text-lg font-bold ${cumColor}`}>
