...
static char *build_prompt(const tb_advisory_context_t *ctx) {
    ...
    snprintf(prompt, prompt_len,
        ...
        "Budget: ~100 USDC. Leverage: 5x. Two active strategies:\n"
        "1) [PRIMARY] Momentum ETH: EMA12/26 crossover + RSI + MACD + volume, "
        "ATR trailing stop (2x initial, 1.5x trail), two-step SL (breakeven at +2%%, lock +2%% at +4%%)\n"
        "2) [SECONDARY] BB Scalping ETH: Bollinger Band mean reversion, "
        "RSI<35 at lower BB → long, RSI>65 at upper BB → short, SL 1.5%%, TP 3%%\n"
        "Emergency close at -12 USDC, hard limit at -15 USDC.\n"
        "\n"
        "Current state:\n%s\n"
        "\n"
        "Provide adjustments as JSON with these optional fields:\n"
        "- momentum_eth: {entry_size, tp_pct, atr_sl_mult, atr_trail_mult, cooldown_sec, pause(bool)}\n"
        "- scalp_eth: {entry_size, sl_pct, tp_pct, rsi_oversold, rsi_overbought, pause(bool)}\n"
        ...
        "- If Fear & Greed < 20, consider pausing scalping (ranging markets less likely)\n"
